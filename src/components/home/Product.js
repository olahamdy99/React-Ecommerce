...
const Product = ({ product, isFavorite }) => {
  ...
  const favorites = useSelector(state => state.favorite.favorites); // Update this line with the correct slice name
  ...
  const handleFavoriteClick = () => {
    if (isFavorite) {
      ...
      if (!existingProduct) {
        ...
        notify(); // Notify only when a new product is added
      }else{
        notifyOfExist()
      }
    }
...
<CiHeart size={32} />
  {/* {isFavorite ? (
    <AiFillHeart size={32} color="red" /> // Render red heart when product is favorite
  ) : (
    <CiHeart size={32} /> // Render gray heart when product is not favorite
  )} */}
                <Toaster
